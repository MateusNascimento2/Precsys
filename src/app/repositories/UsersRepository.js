...
class UsersRepository {
  async findAll(orderBy = 'ASC') {
    ...
    SELECT * FROM precsysapp_demo.users
  ...
  async findById(id) {
    const [row] = await db.query('SELECT * FROM precsysapp_demo.users WHERE users.id = ?', [id]);
    return row;
  }

  // Mudar depois para findByCPFCNPJ
  async findByCPFCNPJ(cpfcnpj) {
    const [row] = await db.query('SELECT * FROM precsysapp_demo.users WHERE cpfcnpj = ?', [cpfcnpj]);
    return row;
  }

  async delete(id) {
    const deleteOp = await db.query('DELETE FROM precsysapp_demo.users WHERE id = ?', [id]);
    return deleteOp;
  }
  ...
  async create({
    ...
    INSERT INTO precsysapp_demo.users(nome, email, password, cpfcnpj, endereco, telefone, qualificacao, obs, ativo, permissao_email, permissao_proposta, permissao_expcartorio, foto, admin)
    VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
